...
function Renderer(canvas, world) {
    ...
    // Pocket holes, drawn at exactly the size the physics uses: a ball whose
    // centre is over this circle has no cloth under it, so what looks like a
    // ball over the hole is a ball on its way down.
    var holeMaterial = new THREE.MeshBasicMaterial({color: 0x07090b});
    world.pockets.forEach(function (p) {
        var hole = new THREE.Mesh(new THREE.CircleGeometry(p.radius, 28), holeMaterial);
